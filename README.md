...
- 情報を共有する前に、情報源・日時・対象・更新状況などを任意で振り返る、AIを使わない静的な「情報を確かめるヒント」を表示する。チェック状態は送信・保存せず、既存のリスク判定にも影響しない。
...
- 確認結果の下には「次にできること」「投稿前セルフチェック」「情報を確かめるヒント」が表示され、投稿・送信前の任意の見直しに利用できます。
...
- **情報を確かめるヒント**: 情報源、一次情報、日時や対象、事実と意見の区別などを利用者自身が振り返る任意の静的チェックリストを日英で表示。AIによる真偽判定は行わず、チェック状態は送信・保存しない
...
(6) ユニットテスト結果が表示されます。`Test Suites` と `Tests` に失敗がなく、実行時点のテストがすべて成功していることを確認します。テスト追加によって件数は変わるため、固定件数ではなく実行結果を正としてください。
...
ライセンスに関するご質問・利用許諾のご相談については、以下のいずれかの方法で、お問い合わせください。

開発者（青木直之）のポートフォリオサイトの「お問い合わせ画面」
[https://portfolio.na0aaooq.com/contact.html](https://portfolio.na0aaooq.com/contact.html)
