...
(2) git cloneしたディレクトリへ移動します。  

```
% cd words-watching-app
```

(3) テスト実行用のパッケージ Jest と jest-environment-jsdom をインストールします。  
...
(4) package.json に対して、Jestとjest-environment-jsdomを追加します。
...
(5) 以下のコマンドを実行し、リポジトリ内の単体テストコード index.test.js で index.html の単体テストを実行します。  

```
% npx jest index.test.js
```

(6) 単体テスト結果が表示されます。テスト結果を確認し、[Tests: xx passed, xx total] とテスト項目が全件成功していることを確認します。
